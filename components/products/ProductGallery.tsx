...
type Props = { productId?: number; mode?: "add" | "edit" };

function SortableImage({ img, onRemove }: { img: GalleryImage; onRemove: (id: string | number) => void })
 {
...
>
  ...
  <button
    ...
    className="absolute right-1 top-1 bg-black/60 p-1 rounded-full text-white flex items-center justify-center z-10"
  >
    <X className="h-3 w-3" />
  </button>
</div>
...
const ProductGallery = forwardRef<any, Props>(({ productId, mode }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    getGalleryData: () => ({
      existing: images.filter(i => i.isExisting).map((img, index) => ({ id: img.id, sort_order: index })),
...
const removeImage = (id: string | number) => {
  ...
  /* When user picks from media library modal */
const handleMediaSelect = async (media: MediaImage) => {
  if (!productId) return;

  // After selection, refetch gallery from backend
  ...
  return (
    <>
      ...
      <MediaLibraryModal
        ...
        folder="gallery"
        productId={productId}      // ← only this product's gallery images shown
