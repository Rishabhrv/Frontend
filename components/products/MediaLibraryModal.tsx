...
export default function MediaLibraryModal({
  ...
  useEffect(() => {
    if (open) {
      ...
      setSearch("");
      setPage(1);
      // 🔥 FIX: We DO NOT clear setTempImages([]) here anymore. 
      // This allows session uploads to persist when closing/reopening!
      setTab("library");
      fetchImages(1);
    }
  }, [open, fetchImages]);
  ...
  /* Auto-fetch details when the active image changes */
  useEffect(() => {
    if (activeImage) {
              ...
              <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-3">
                  {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-40 text-gray-400">
                      ...
                      <p className="text-sm">{!productId && tempImages.length === 0 ? "Upload images here to assign them to your product" : "No images found for this product"}</p>
                      {!productId && tempImages.length === 0 && (
                    ...
                    <div className="grid grid-cols-5 sm:grid-cols-7 md:grid-cols-8 gap-1.5">
                      {displayImages.map((img) => {
                        ...
                        return (
                          ...
                          >
                            <img src={`${API_URL}${img.url}`} alt={img.alt_text || img.filename} className="w-full h-full object-cover" loading="lazy" />
                            {isSel && (
                ...
                {!loading && totalPages > 1 && (
                  <div className="shrink-0 flex items-center justify-between px-4 py-2.5 border-t border-gray-200 bg-gray-50">
                    <p className="text-xs text-gray-500">Page {page} of {totalPages}<span className="ml-2 text-gray-400">({total} total)</span></p>
                    <div className="flex items-center gap-1">
                      <button type="button" onClick={() => goToPage(page - 1)} disabled={page === 1} className="p-1.5 rounded border border-gray-300 text-gray-500 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><ChevronLeft className="w-3.5 h-3.5" /></button>
                      {getPaginationRange().map((p, i) =>
                        p === "…" ? (
                          <span key={`ellipsis-${i}`} className="px-1.5 text-xs text-gray-400">…</span>
                        ) : (
                          <button key={p} type="button" onClick={() => goToPage(p as number)} className={`min-w-[28px] h-7 px-1.5 rounded text-xs font-medium border transition-colors ${page === p ? "bg-blue-600 text-white border-blue-600" : "border-gray-300 text-gray-600 hover:bg-gray-100"}`}>
                            {p}
                          </button>
                        )
                      )}
                      <button type="button" onClick={() => goToPage(page + 1)} disabled={page === totalPages} className="p-1.5 rounded border border-gray-300 text-gray-500 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><ChevronRight className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                )}
              </div>
              ...
              {activeImage && (
                <div className="w-60 border-l border-gray-200 overflow-y-auto shrink-0 flex flex-col">
                  <div className="p-4 border-b border-gray-100 bg-gray-50">
                    ...
                    <img src={`${API_URL}${activeImage.url}`} alt={activeImage.filename} className="w-full rounded border border-gray-200 object-cover max-h-36" />
                  </div>
                  <div className="p-4 space-y-2 text-xs text-gray-600 flex-1">
                    ...
                    {activeImage.created_at && (
                      <p className="text-gray-400">{new Date(activeImage.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p>
                    )}
