...
type ReadyToGoProductAttributesProps = {
  initialAttributes?: { name: string; value: string }[];
  error?: string;
  onValidChange?: () => void;
};
...
const ReadyToGoProductAttributes = forwardRef<any, ReadyToGoProductAttributesProps>(
  ({ initialAttributes, error, onValidChange }, ref) => {
    ...
    // Fetch available attributes list for the dropdown
    useEffect(() => {
        ...
        setAttributes((prev) => {
          ...
          parsed.forEach((importedAttr: { name: string; value: string }) => {
            ...
            if (existingIndex >= 0) {
              ...
              newAttrs[existingIndex] = { ...newAttrs[existingIndex], values: safeValue };
            } else {
              // Add as a new dynamic attribute
              newAttrs.push({
      ...
      <div className={`bg-white rounded-xl border p-6 border-gray-300`}>
        ...
        <div className="flex items-center gap-2 mb-4">
          <button
            type="button"
            onClick={addNew}
            className="rounded border border-gray-300 px-3 py-1.5 text-sm cursor-pointer hover:bg-gray-50 font-medium text-gray-700 transition-colors"
          >
            Add new
          </button>
            ...
            <div key={attr.id} className="border border-gray-300 rounded overflow-hidden">
              ...
              {attr.open && (
                <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs mb-1 text-gray-600 font-medium">Name</label>
                    <input
                      type="text"
                      value={attr.name}
                  ...
                  <div>
                    <label className="block text-xs mb-1 text-gray-600 font-medium">Value(s)</label>
                    <textarea
                      rows={2}
...
export default ReadyToGoProductAttributes;