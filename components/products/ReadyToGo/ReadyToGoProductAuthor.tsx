...
// Aggressive String Normalizer for perfect matching between Python and Express DBs
...
const ReadyToGoProductAuthor = forwardRef<any, ReadyToGoProductAuthorProps>(
  ({ initialAuthors, error, onValidChange }, ref) => {
    ...
    // 1. Fetch DB Authors ONCE on mount
    useEffect(() => {
      let isMounted = true;
    ...
    // 2. Process imported authors: Auto-Create & Download Photo as Blob to store in Node.js
    useEffect(() => {
      ...
      if (parsedInitialAuthors && parsedInitialAuthors.length > 0) {
        hasImportedRef.current = true; // Lock it so it only imports once
        ...
        const processImports = async () => {
          ...
          for (const imported of parsedInitialAuthors) {
            ...
            if (expressAuthor) {
              ...
              // 👇 If author exists but lacks photo, download from Python & upload to Node.js 👇
              ...
              // 👇 If author is MISSING, download image FIRST, then POST to Node.js 👇
              ...
              if (fileToUpload) {
                formData.append("profile_image", fileToUpload); // Will trigger your Multer storage
              }
          ...
          // Re-fetch authors to grab the new local `/uploads/authors/...` paths generated by Express
    ...
    }, [dbAuthorsFetched, initialAuthorsStr, authors, onValidChange]);
              ...
              >
                <div className="flex items-center gap-2">
                  <div className="h-8 w-8 rounded-full overflow-hidden bg-gray-300 flex items-center justify-center border border-gray-400">
                    ...
                    {a.profile_image ? (
                      // Display the Local Express Upload
                      <img
                      ...
                      // Stream from Python temporarily while upload happens
                      <img
