...
export default function SingleProductPage({ product }: { product: Product }) {
  ...
  // Just remove anything related to slug or setProduct
...
<div className="mt-14 border-y border-gray-300 py-8">
  ...
  <div
    ...
    dangerouslySetInnerHTML={{
      __html: showFullDesc || !isLongDescription
        ? product.description
        : shortDescription + "...",
    }}
  />
      ...
      {product.authors.map((a) => (
        <div key={a.id} className="flex items-start gap-4 rounded p-3">
          ...
          <div className="flex-shrink-0 w-[80px] h-[80px]">
            <Link href={`/author/${a.slug}`}>
              <Image
                ...
                width={60}
                height={60}
                className="w-[80px] h-[80px] rounded-full object-cover cursor-pointer hover:opacity-80 transition-opacity"
                unoptimized
                alt={a.name}
              />
            </Link>
          ...
          <div className="flex-1 min-w-0">
            ...
            <Link
              href={`/author/${a.slug}`}
              className="font-medium block hover:underline hover:text-black transition-colors"
            >
              {a.name}
            </Link>
