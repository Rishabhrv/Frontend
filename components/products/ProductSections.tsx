    ...
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <button
          ...
          className={`p-1 rounded-md transition-colors cursor-pointer ${isOpen ? "bg-blue-50 text-blue-600" : "text-gray-400 hover:text-blue-600 hover:bg-gray-50"}`}
          title={`${labelPrefix} Styles`}
        >
      ...
      {isOpen && (
        <div className="mt-2 p-2.5 bg-gray-50 border border-gray-200 rounded-lg space-y-3">
          {/* Margin */}
          <div>
            <span className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">
              {labelPrefix} Margin
            </span>
            <div className="grid grid-cols-4 gap-1.5">
              {(["Top", "Right", "Bottom", "Left"] as const).map((dir) => (
                <input
                  ...
                  type="text"
                  placeholder={dir}
                  value={styles?.[`margin${dir}`] || ""}
                  onChange={(e) => updateStyle(`margin${dir}`, e.target.value)}
                  className="w-full rounded-md border border-gray-200 px-1.5 py-1 text-[10px] text-gray-700 placeholder:text-gray-400 focus:outline-none focus:border-blue-400 transition-all text-center"
                  title={`Margin ${dir} (e.g. 10px)`}
                />
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-4 gap-1.5">
              {(["Top", "Right", "Bottom", "Left"] as const).map((dir) => (
                <input
                  ...
                  type="text"
                  placeholder={dir}
                  value={styles?.[`padding${dir}`] || ""}
                  onChange={(e) => updateStyle(`padding${dir}`, e.target.value)}
                  className="w-full rounded-md border border-gray-200 px-1.5 py-1 text-[10px] text-gray-700 placeholder:text-gray-400 focus:outline-none focus:border-blue-400 transition-all text-center"
                  title={`Padding ${dir} (e.g. 10px)`}
                />
              ))}
            </div>
          </div>
...
const ProductSections = forwardRef(
  ({ productId, initialSections }: Props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      getSectionsData: () =>
        sections.map((s) => ({ type: s.type, data: s.data })),
    ...
    const addSection = (type: SectionType) => {
      ...
      setSections((prev) => [
        ...prev,
        { uid: nextUid(), type, data: { ...(def?.defaultData || {}) } },
      ]);
      setPickerOpen(false);
    };
    ...
    const updateField = (uid: string, field: string, value: any) =>
      setSections((prev) =>
        prev.map((s) =>
          s.uid === uid ? { ...s, data: { ...s.data, [field]: value } } : s,
        ),
      );
    ...
    const removeSection = (uid: string) => {
      Object.keys(imageFilesRef.current).forEach((key) => {
        if (key.startsWith(`__FILE__${uid}__`))
    ...
    const ImageUploadZone = ({
      ...
      return (
        <div
          className={`relative group ${height} rounded-xl overflow-hidden border-2 border-dashed border-gray-200 bg-gradient-to-br from-gray-50 to-slate-100 transition-all duration-200 hover:border-blue-400 hover:shadow-sm`}
        >
          {src ? (
            <>
              ...
              <input
                ...
                hidden
                onChange={(e) =>
                  onFileSelect(uid, field, e.target.files?.[0] || null)
                }
              />
            </>
          ) : (
    ...
    const ItemImageUploadZone = ({
      ...
      return (
        <div
          className={`relative group ${height} rounded-lg overflow-hidden border-2 border-dashed border-gray-200 bg-gradient-to-br from-gray-50 to-slate-100 transition-all duration-200 hover:border-blue-400 hover:shadow-sm`}
        >
          {src ? (
            <>
              <img src={src} className="h-full w-full object-cover" alt="" />
        ...
        <div className="p-5">
          ...
          <div className="space-y-3">
            {sections.map((section, index) => {
              ...
              const meta =
                SECTION_META[section.type] || SECTION_META.single_image;
              ...
              return (
                <div
                  key={section.uid}
                  className={`border rounded-xl overflow-hidden transition-all duration-200 ${isCollapsed
                    ? "border-gray-200"
                ...
                >
                  {/* ── Section header bar ─────────────────────── */}
                  <div
                    className={`flex items-center justify-between px-4 py-2.5 transition-colors ${isCollapsed
                      ? "bg-gray-50"
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      {/* Type badge */}
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium ${meta.bg} ${meta.color} ${meta.border} border`}
                      >
                  ...
                  {!isCollapsed && (
                    <div className="p-5 bg-white">
                      ...
                      <div className="mb-5">
                        <FieldWithStyles
                          label="Section Title"
                          styles={section.data.titleStyles}
                          onChange={(val: any) =>
                            updateField(section.uid, "titleStyles", val)
                          }
                          labelPrefix="Title"
                        >
                          <input
                            ...
                            onChange={(e) =>
                              updateField(section.uid, "title", e.target.value)
                            }
                      ...
                      {section.type === "single_image" && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div>
                            <FieldWithStyles
                              label="Image"
                              styles={section.data.imageStyles}
                              onChange={(val: any) =>
                                updateField(section.uid, "imageStyles", val)
                              }
                              labelPrefix="Image"
                            >
                              <ImageUploadZone
                          ...
                          <div className="space-y-3">
                            <div>
                              <FieldWithStyles
                                label="Heading"
                                styles={section.data.headingStyles}
                                onChange={(val: any) =>
                                  updateField(section.uid, "headingStyles", val)
                                }
                                labelPrefix="Heading"
                              >
                                <input
                          ...
                          {(["Left", "Right"] as const).map((side) => {
                            ...
                            return (
                              <div key={side} className="space-y-2">
                                <FieldWithStyles
                                  label="{side} Image"
                                  styles={section.data[`${imgField}Styles`]}
                                  onChange={(val: any) =>
                                    updateField(
                                      section.uid,
                                      `${imgField}Styles`,
                                ...
                                <FieldWithStyles
                                  label="Caption"
                                  optionalText="(max 35 words)"
                                  styles={section.data[`${capField}Styles`]}
                                  onChange={(val: any) =>
                                    updateField(
                                      section.uid,
                                      `${capField}Styles`,
                            ...
                            <div className="flex flex-wrap gap-2">
                              {[
                                ...
                                <button
                                  ...
                                  className={`px-4 py-2 rounded-lg text-xs font-medium border transition-all cursor-pointer ${(section.data.layout || "image-left") ===
                                    opt.value
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div className="space-y-3">
                              ...
                              <div>
                                <FieldWithStyles
                                  label="Content"
                                  styles={section.data.contentStyles}
                                  onChange={(val: any) =>
                                    updateField(
                                      section.uid,
                                      "contentStyles",
